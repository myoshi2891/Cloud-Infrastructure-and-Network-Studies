...
export const DIAGRAMS: Record<DiagramId, string> = {
    ...
    "diag-12": "flowchart TB\n    G1[\"1G<br/>アナログ音声\"] --> G2[\"2G<br/>デジタル音声 + SMS<br/>GSM / CDMA\"]\n    G2 --> G3[\"3G<br/>パケット交換データ<br/>UMTS / CDMA2000\"]\n    G3 --> G4[\"4G LTE<br/>オールIP<br/>フラットなRAN構造\"]\n    G4 --> G5[\"5G<br/>超低遅延 + 超高速 + 大量接続<br/>mMTC / URLLC\"]\n    G5 --> G5A[\"5G-Advanced<br/>Release 19 中心 / Release 20 継続\"]\n    G5A --> G6[\"6G<br/>Release 20 で研究（スタディ）段階\"]\n\n    classDef highlightFill fill:#1a2f52,stroke:#7c9eff,color:#eaf0ff\n    class G5,G5A,G6 highlightFill",
    "diag-13": "flowchart TD\n    subgraph RRC_States[\"RRC 状態遷移（5G NR）\"]\n        direction TB\n        IDLE[\"【RRC_IDLE】<br/>待機中 / バッテリー消費最小<br/>通信不可・コンテキストなし\"]\n        CONN[\"【RRC_CONNECTED】<br/>データ送受信中 / 最も高速<br/>バッテリー消費大\"]\n        INACT[\"【RRC_INACTIVE】（5G NR）<br/>省電力かつ即時再開可能<br/>RANコンテキスト保持\"]\n\n        IDLE -->|\"データ送信要求<br/>接続確立（数百ms〜数秒）\"| CONN\n        CONN -->|\"一定時間無通信でタイムアウト<br/>コンテキスト解放\"| IDLE\n        CONN -->|\"短い無通信で移行\"| INACT\n        INACT -->|\"データ発生：RRCResume<br/>（数十msで高速復帰）\"| CONN\n        INACT -->|\"さらに長い無通信で解放\"| IDLE\n    end\n\n    classDef idleFill fill:#1e293b,stroke:#64748b,color:#e2e8f0\n    classDef connFill fill:#1a2f52,stroke:#7c9eff,color:#eaf0ff\n    classDef inactFill fill:#1e3a34,stroke:#34d399,color:#d1fae5\n    class IDLE idleFill\n    class CONN connFill\n    class INACT inactFill",
    ...
    "diag-22": "sequenceDiagram\n    autonumber\n    participant B as ブラウザ\n    participant S as サーバー (別オリジン)\n\n    rect rgb(26, 47, 82)\n        Note over B,S: 【ステップ1：プリフライト検証（+1 RTT）】\n        B->>S: OPTIONS /api/data<br/>Origin: https://app.example.com<br/>Access-Control-Request-Method: POST\n        S-->>B: 204 No Content<br/>Access-Control-Allow-Origin: https://app.example.com<br/>Access-Control-Allow-Methods: POST, GET\n    end\n\n    rect rgb(30, 41, 59)\n        Note over B,S: 【ステップ2：本リクエスト送信】\n        B->>S: POST /api/data (Content-Type: application/json)\n        S-->>B: 200 OK + レスポンスデータ\n    end",
    ...
    "diag-31": "flowchart TB\n    R19[\"Release 19（2025年完了）<br/>5G-Advanced主要機能の策定\"] --> R20[\"Release 20（2026年策定中）<br/>5G-Advanced拡張 + 6G研究（スタディ）開始\"]\n    R20 --> R21[\"Release 21（2028〜2029年見込み）<br/>初の商用向け6G仕様策定\"]\n    R21 --> Commercial[\"商用6G網（2030年前後）<br/>テラヘルツ帯・AIネイティブ通信の実現\"]\n\n    classDef highlightFill fill:#1a2f52,stroke:#7c9eff,color:#eaf0ff\n    class R20 highlightFill",
