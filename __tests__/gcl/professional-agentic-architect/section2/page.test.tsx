// __tests__/gcl/professional-agentic-architect/section2/page.test.tsx
// @vitest-environment jsdom
import { render } from '@testing-library/react';
import { describe, expect, it, test, vi } from 'vitest';
import inventory from '@/docs/migration-inventory/professional-agentic-architect-section2.json';
import Page from '@/app/gcl/professional-agentic-architect/section2/page';
...
defineMigrationSuite(
    'Professional Agentic Architect Section 2 — 全量移行検証',
    Page,
    inventory,
);

describe('Professional Agentic Architect Section 2 — 詳細仕様検証', () => {
    const renderPage = () => {
    ...
    it('チェックリストカード内に12件のチェックボックス項目が存在し、動的カウントアップの基盤がある', () => {
        const container = renderPage();
        const checklist = container.querySelector('.checklist-card');
        expect(checklist).not.toBeNull();
        const checkboxes = checklist?.querySelectorAll('input[type="checkbox"]');
        expect(checkboxes && checkboxes.length).toBe(12);
        const labels = checklist?.querySelectorAll('label');
        expect(labels && labels.length).toBe(12);
    });

    it('参考文献カード(ref-card)が29件存在し、外部リンクが正しく設定されている', () => {
        const container = renderPage();
    ...
    it('全6件のテーブルが存在し、すべてtheadとth[scope="col"]を持つ', () => {
        const container = renderPage();
    ...
    it('page.css 内でサイドバー契約およびリストマーカー保持スタイルが定義されている', async () => {
        ...
        const pageCss = readFileSync(
            join(
                process.cwd(),
                'app/gcl/professional-agentic-architect/section2/page.css',
            ),
            'utf8',
        );
    ...
    it('DIAGRAMS 内の全7図が構文エラーなく parse できること', async () => {
        const { DIAGRAMS } = await import(
            '@/app/gcl/professional-agentic-architect/section2/constants'
        );
        ...
        for (const [id, chart] of Object.entries(DIAGRAMS)) {
            const cleanChart = chart.trim().startsWith('sequenceDiagram')
                ? chart.replace(/<br\s*\/?>/gi, ' ')
                : chart.replace(/<br\s*\/?>/gi, '\n');
