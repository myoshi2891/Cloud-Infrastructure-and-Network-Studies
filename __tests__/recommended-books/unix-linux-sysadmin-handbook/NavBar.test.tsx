// __tests__/recommended-books/unix-linux-sysadmin-handbook/NavBar.test.tsx
// @vitest-environment jsdom
import { fireEvent, render, screen } from '@testing-library/react';
import { act } from 'react';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
import { NavBar } from '@/app/recommended-books/unix-linux-sysadmin-handbook/NavBar';
import { NAV_ITEMS } from '@/app/recommended-books/unix-linux-sysadmin-handbook/constants';
...
describe('unix-linux-sysadmin-handbook NavBar', () => {
    ...
    it('モバイルトグルボタンで開閉状態が切り替わる', () => {
        ...
        const sidebar = screen.getByRole('complementary', { name: 'サイドバー目次' });
    ...
    it('リンククリック時にモバイルサイドバーが閉じる', () => {
        ...
        const link = linkFor(navItemAt(1).id);
        fireEvent.click(link);
    ...
    it('hashchange イベントで activeId が更新される', () => {
        render(<NavBar />);
        const targetItem = navItemAt(4);
        ...
        const activeLink = linkFor(targetItem.id);
        expect(activeLink.classList.contains('active')).toBe(true);
    });
});
