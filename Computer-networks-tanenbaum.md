...
- [ステップ0: 全体像を掴む ― インターネットはどう成り立っているか](#step0)
  - [インターネットを構成する要素](#step0-1)
  - [プロトコル階層化という考え方](#step0-2)
  - [カプセル化: データが層を降りていく仕組み](#step0-3)
- [ステップ1: 物理層 ― ビットを電気信号・光・電波に変える](#step1)
  - [伝送媒体の比較](#step1-1)
  - [信号化・多重化の基礎](#step1-2)
- [ステップ2: データリンク層 ― フレーム化と誤り検出でリンクを渡す](#step2)
  - [フレーミングと誤り検出](#step2-1)
  - [イーサネット(Ethernet)フレームの構造](#step2-2)
  - [スイッチの学習と転送(自己学習ブリッジ)](#step2-3)
  - [VLAN(仮想LAN)による論理分割](#step2-4)
- [ステップ3: メディアアクセス制御(MAC)サブレイヤー ― 誰が話す番かを決める](#step3)
  - [CSMA/CD(有線イーサネットの歴史的方式)](#step3-1)
  - [CSMA/CA(無線LANで使われる方式)](#step3-2)
  - [Wi-Fi世代の比較](#step3-3)
- [ステップ4: ネットワーク層 ― 異なるネットワークをまたいで届ける](#step4)
  - [IPv4アドレッシングとCIDR](#step4-1)
  - [サブネッティングの手順](#step4-2)
  - [IPv6が必要な理由と基本構造](#step4-3)
  - [ルーティングアルゴリズムの2つの系統](#step4-4)
  - [NAT(ネットワークアドレス変換)](#step4-5)
  - [ICMPとインターネットの経路検証](#step4-6)
- [ステップ5: トランスポート層 ― エンドツーエンドの信頼性](#step5)
  - [TCPの3ウェイハンドシェイク](#step5-1)
  - [TCPコネクションの状態遷移](#step5-2)
  - [フロー制御と輻輳制御](#step5-3)
  - [TCPとUDPの比較](#step5-4)
- [ステップ6: アプリケーション層 ― 人とアプリのためのプロトコル](#step6)
  - [DNS(Domain Name System)による名前解決](#step6-1)
  - [HTTP/HTTPSの基本](#step6-2)
  - [メールプロトコルの概要](#step6-3)
  - [DHCPによる自動設定](#step6-4)
- [ステップ7: ネットワークセキュリティ ― 守るべきものと手段](#step7)
  - [TLSによる暗号化通信](#step7-1)
  - [VPN(Virtual Private Network)](#step7-2)
  - [ファイアウォールとDMZアーキテクチャ](#step7-3)
  - [耐量子暗号(PQC)への移行](#step7-4)
- [ステップ8: 2026年の最新動向 ― 今のインターネットはどう変わったか](#step8)
- [ステップ9: トラブルシューティングの基本 ― 現場で使う一次切り分け](#step9)
  - [よく使う一次切り分けコマンド](#step9-1)
- [学習ロードマップ](#roadmap)
...
データリンク層は、同一のリンク(同一セグメント)上にある隣接ノード間で、ビット列を「フレーム」という単位にまとめ、誤り検出を行ったうえで受け渡す役割を担います。ネットワーク層のIPアドレスとは異なり、この層では**MACアドレス(Media Access Control address)**というリンク層アドレス(一般的には48ビット)が使われます。48ビットのEUI-48が主流ですが、IEEE 802.15.4などではEUI-64が使われ、またNICに焼き付けられた値だけでなく、管理者による設定やプライバシー保護目的のランダム化によって変更された値が使われることもあります。
...
| Wi-Fi 7 | 802.11be | Wi-Fi Alliance認証2024年1月、IEEE承認2024年9月、標準公開2025年7月 | 320MHz | MLO(複数リンク同時利用)、4K-QAM |
...
UDPの上に信頼性・輻輳制御・暗号化を独自に構築したのが、後述するQUIC(RFC 9000)です。QUIC自体はトランスポートプロトコルであり、HTTP/3はその上で動作するアプリケーション層プロトコルです。QUICはユーザースペースのライブラリとして実装・更新できるため、輻輳制御アルゴリズムの改善サイクルを速められる利点があります。
...
| HTTP/3 | QUIC(UDPベース) | QUICがストリーム単位で順序制御するため、パケット損失の影響が他のストリームへ波及しにくい。コネクション確立も高速化 |
...
- APIのタイムアウト設計では、通信区間ごとに要する時間を分けて見積もる。HTTP/1.1・HTTP/2(TCP)ではDNS解決・TCP確立・TLSハンドシェイク・リクエスト処理を、HTTP/3(QUIC)ではDNS解決・QUIC接続確立(トランスポートとTLS 1.3ハンドシェイクが統合される)・リクエスト処理を、それぞれ個別に見積もる
...
| Wi-Fi | Wi-Fi 7(IEEE 802.11be)がIEEE承認2024年9月・標準公開2025年7月を経て正式標準化され、2026年通年で全世界の対応機器出荷が約11億台規模に到達する見込み。次世代のWi-Fi 8(802.11bn)は速度よりも複数AP協調やローミングの安定性を重視する方向で、標準化目標は2028年 | Network World |
...
flowchart TD
    ...
    Q6 -->|UDP/QUIC| Q6U{"プロトコル固有の応答が返るか<br/>(DNS: dig @宛先 名前 +notcp +ignore / HTTP/3: curl --http3-only -v)<br/>汎用の nc -u は無応答でも判定不能"}
...
| `nc -u` / `nmap -sU` | UDPポートへの汎用プローブ。無応答はopen\|filtered(到達したが無反応か、途中で遮断されたかを区別できない)であり、結果は判定不能として扱う | トランスポート層(UDP) |
| `dig @<宛先> <名前>` | DNS(UDP/53)へ実際に問い合わせて応答有無を確認 | アプリケーション層(DNS)経由のUDP疎通 |
| `curl --http3-only -v` | HTTP/3(QUIC over UDP/443)のハンドシェイクと応答を確認(`--http3`と異なりHTTP/2・HTTP/1.1へフォールバックしない) | トランスポート層(QUIC)〜アプリケーション層 |
...
- 切り分けの前に宛先サービスがTCPかUDP/QUIC(DNS、SNMP、syslog、HTTP/3など)かを確認する。UDPはコネクションを確立しないため、`telnet`や`nc`(TCP)が失敗しても、それはUDPサービスのトランスポート障害の根拠にはならない
- UDPベースのサービスはプロトコル固有の応答で判定する。DNSなら`dig @<宛先> <名前>`、HTTP/3なら`curl --http3-only -v`でQUICハンドシェイクの成否を見る。`--http3`はHTTP/2・HTTP/1.1へ暗黙にフォールバックするため、QUIC疎通の判定には必ず`--http3-only`を使う。`curl --http3-only`が失敗して`curl --http2`が成功する場合は、経路上のUDP/443遮断と即断せず、(1) UDP/443が実際に通るか、(2) `curl -V`で手元のcurlがHTTP/3(QUIC)対応ビルドか(`HTTP3`機能の有無)、(3) 対象サーバーがそもそもQUIC/HTTP/3を提供しているか(`Alt-Svc: h3=...`の応答有無など)、の3点を切り分ける。汎用の`nc -u`の無応答は判定不能であり、遮断の根拠にはならない
